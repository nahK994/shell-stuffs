...
while true; do
    read -p "(13/${numberOfTasks}) Want to install nvm? (y/N): " nvm
    if [[ $nvm =~ ^[yY]$ ]]; then
...
while true; do
    read -p "(14/${numberOfTasks}) Want to install git? (y/N): " git
    if [[ $git =~ ^[yY]$ ]]; then
...
while true; do
    read -p "(15/${numberOfTasks}) Want to install docker? (y/N): " docker
    if [[ $docker =~ ^[yY]$ ]]; then
        sudo apt-get update -y
...
while true; do
    read -p "(16/${numberOfTasks}) Want to install python3.12 and pip3 (python3.12)? (y/N): " python
    if [[ $python =~ ^[yY]$ ]]; then
...
if ! which pip > /dev/null; then
    while true; do
        read -p "(17/${numberOfTasks}) Want to install pip? (y/N): " pip
        if [[ $pip =~ ^[yY]$ ]]; then
